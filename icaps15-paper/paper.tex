...
100,000 simulated frames, except for UCT limited to 500 rollouts, 300 frames deep instead. Average scores and CPU times per action selection shown.
...
Our experimental setup  follows the one in BNVB with two exceptions: rather than
considering $10$ runs per games, we considered just $5$ due to time constraints (*** ???),
and  a maximum budget of  $150,000$ simulated frames is applied to all the  algorithms, 
including  \UCT\ that is limited to  $500$ base policy rollouts of depth $300$.
...
The alternative is to ``reopen'' such nodes.  The same change is actually in \BFS\ to ensure that, 
if given enough time, \BFS\ will actually find the optimal paths. The code used for \IW\ and \BFS\ in the
