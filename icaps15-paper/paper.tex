...
\input{games-table}
We tested \IW(1) and \BFS\ over $35$ of the 55 games considered in
...
Each run can take up to 24 hours of CPU time and there are 5 runs per game and algorithm. 
...
The scores for \BRFS\ and \UCT\ are from BNVB.  
...
except \IW(1), that due to its agressive pruning does not always use
...
to the emulator are done for transitions that are cached in that subtree, 
...
that also selects  actions every 5 frames. Since the lookahead budget 
is given by a maximum number of (new) frames, and the time is mostly taken by 
calls to the emulator, this may not be the best choice  for \IW(1)\ 
and \BFS\ that are therefore not exploiting all the options afforded
by the budget. We hope to fix this gap in the future.
...
Table~\ref{performance} shows that both $\IW(1)$ and \BFS\  outperform
...
More interestingly,  \IW(1) outscores \UCT\  in 17 of the 35 games,
while \BFS\ outscores \UCT\ in 18.\footnote{These numbers can actually
...
\cite{deep-mind-atari} in the $7$ games considered, and
\BFS\ does so in $6$ of the $7$ games. Likewise, in \Freeway, \Berzerk, \Bowling, \Montezuma,
\StarGunner, and \Venture, both $\IW(1)$ and \BFS\ attain a better
score than the baseline semi-random algorithm \emph{Perturb} in
\cite{bellemare:jair2013}.
...
In \Freeway, 12 out of 18 possible actions have an effect:
6 actions behave as move-up (up-right, up-left, up-fire, up-right-fire, up-left-fire) , 
6 as  move-down (down-right, down-left, down-fire, down-right-fire, down-left-fire), 
and 6 actions do nothing.  \emph{Perturb}  does well in this domain 
when the selected fixed action is move-up.  As seen 
in Table~\ref{performance}, \UCT\ does not manage to take any chicken
across the highway. The reason that  \UCT\ performs so erratically 
and the rollouts do not manage to collect any reward, is that there  is a $1/3$ probability of  
choosing a move up action,  and   200 move up actions are required  in order to collect a reward. 
...
\IW\ does not have this limitation and is best in \Freeway.
...
our planning algorithms don't care much about losing lifes
until there is just one life left, when they play much better.
This can be seen in the videos mentioned above, and points
to a simple form of learning that would be useful both
...
encoded procedurally (as in simulators) as long as the structure of the states 
is known. Regarding the Atari games, two problems of  all the  planning methods
in ALE are first,  that by using the RAM state of the Atari console they are ``cheating'', 
as they use information that is not available to the human player, and second, 
that they do not get better with experience. In the future, we would like to explore 
whether  variations of these algorithms  can be used to play  from the information in the  
screen state as opposed to the RAM state, and potential uses of the  new ideas
for learning to improve performance. 
