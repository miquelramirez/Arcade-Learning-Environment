...
Time and  memory outs  after 30 minutes and 2 GB resp. From \cite{nir:ecai2012}.
}
\label{table1}
\end{table}
%%%%
...
%%Explain: IW(1), 2BFS, and possible IW11. Also, Dijkstra postprocessing over IW(1) graph if ran.
...
IW(2), $(n' \times D')^2 \times b$ states, which for $n'=1024$ and $D=2$
...
both queues. The exception are the nodes with accumulated reward 0 that are only
placed in the first queue.
We refer to this best-first algorithm below, as 2BFS.

In the experiments below, we use IW(1) with two simple variations.
First, in the breadth-first search, we generate the children in 
random order. This makes the execution that results from the
...
Second, a discount factor $\gamma=0.995$ is added
for discounting future rewards like in UCT. For this,
each state $s$ keeps its depth d(s) in the IW(1) tree,
and if state s' is the child of state s and action a, 
$R(s')$ is set to $R(s) + \gamma^d(s´) r(a,s)$.
A discount factor less than 1 results in a slight
preference for rewards that can be reached earlier, 
which is reasonable  in the on-line setting.

%% NIR: IW11 Don't need to be explained. Maybe in Future Work?
...
\end{tabular}
 ...
 \caption{\small Random baseline, \BRFS, $\IW(1)$, \BFS, \UCT~performance over 5 runs,
   ...
   frames. \textit{T} is the average CPU time per call to each action selection algorithm, \textit{S} is the average score achieved,
...
though classical planning is PSPACE-complete \cite{bylander:complexity}. The partial answer is that 
...
all the games featured in \cite{bellemare:jair2013}, and to add more runs per  game. 
